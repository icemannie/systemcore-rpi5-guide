...
class PatcherGUI:
    ...
    def _build_layout(self) -> None:
        ...
        # Path overrides for kernel / flash-pico / regdb
        ...
        self._file_row(paths, 0, "Kernel tree:", self.kernel_var, self._pick_kernel,
                       is_dir=True)
        self._file_row(paths, 1, "flash-pico.sh:", self.flash_pico_var, self._pick_flash_pico)
        self._file_row(paths, 2, "wireless-regdb .deb:", self.regdb_var, self._pick_regdb)
        ...
        self.log_widget = scrolledtext.ScrolledText(
            log_frame, height=18, wrap="word", font=("Courier New", 9)
        )
    ...
    def _pick_kernel(self) -> None:
        path = filedialog.askdirectory(title="Path to built rpi-linux/ tree")
        if path:
